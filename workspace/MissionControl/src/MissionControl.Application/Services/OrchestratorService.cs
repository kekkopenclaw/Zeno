...
public class OrchestratorService
{
    ...
    public async Task TickAsync(int projectId)
    {
        ...
        foreach (var todoTask in allTasks.Where(t => t.Status == TaskItemStatus.Todo))
        {
            ...
            var orchestrator = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle); // dynamic role-agnostic
            if (orchestrator == null)
            {
                ...
                orchestrator = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle); // dynamic role-agnostic
            }
        ...
        foreach (var orchTask in allTasks.Where(t => t.Status == TaskItemStatus.Orchestration))
        {
            ...
            if (orchestrator != null) // dynamic role-agnostic
            {
            ...
            var architect = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle && a.Id != orchestrator?.Id); // any next Idle agent except previous
            if (architect != null)
            {
        ...
        foreach (var archTask in allTasks.Where(t => t.Status == TaskItemStatus.Architecture))
        {
            ...
            var bulma = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle && a.Id != architect?.Id); // dynamic role-agnostic
            if (bulma != null)
            {
        ...
        foreach (var codingTask in allTasks.Where(t => t.Status == TaskItemStatus.Coding))
        {
            ...
            if (agentWorkCompleted)
            {
                ...
                var piccolo = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle && a.Id != coder?.Id); // dynamic role-agnostic
                if (piccolo != null)
                {
        ...
        foreach (var secTask in allTasks.Where(t => t.Status == TaskItemStatus.Security))
        {
            ...
            var dende = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle && a.Id != cell?.Id); // dynamic role-agnostic
            if (dende != null)
            {
        ...
        foreach (var enfTask in allTasks.Where(t => t.Status == TaskItemStatus.Enforcement))
        {
            ...
            var zeno = agents.FirstOrDefault(a => a.Status == AgentStatus.Idle && a.Id != jiren?.Id); // dynamic role-agnostic
            if (zeno != null)
            {
        ...
        foreach (var doneTask in allTasks.Where(t => t.Status == TaskItemStatus.Done))
        {
            ...
            if (trunks != null && trunks.Status == AgentStatus.Working) // Role-agnostic
            {
                trunks.Status = AgentStatus.Idle;
                await _agentRepository.UpdateAsync(trunks);
                await _notifier.NotifyAgentStartedAsync(AgentService.MapToDto(trunks));
